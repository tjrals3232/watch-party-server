    ...
    ensureDOMReady(() => {
      ...
      if (joinBtn) {
        joinBtn.onclick = () => {
          ...
          chrome.storage.local.get(["tpRoom", "tpNick"], (prev) => {
            const isSame = (prev.tpRoom === room && prev.tpNick === nick);

            // Always update storage (even if same, to be safe/consistent, though chrome might not fire onChanged)
            ...
            // Only emit manually if we think onChanged won't fire (values are same)
            // or just let the server handle dedup (safest).
            // But to be cleaner:
            if (isSame && socket && socket.connected) {
      ...
      socket.on("connect", () => {
        ...
        loadRoomNick();
        // [Fix] Removed duplicate emit here because loadRoomNick() already handles it
      });
